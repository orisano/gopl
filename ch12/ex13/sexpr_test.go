...
func TestMarshal(t *testing.T) {
	...
	t.Run("Tag", func(t *testing.T) {
		s := &struct {
			Name1 string
			Name2 string `sexpr:"foo"`
		}{
			Name1: "a",
			Name2: "bar",
		}
		expected := `((Name1 "a") (foo "bar"))`
		b, err := Marshal(s)
		if err != nil {
			t.Fatal(err)
		}
...
func TestUnmarshal(t *testing.T) {
	...
	t.Run("Tag", func(t *testing.T) {
		...
		var s Tag
		if err := Unmarshal([]byte(`((Name1 "a") (foo "bar"))`), &s); err != nil {
			t.Fatal(err)
		}
		expected := Tag{
			Name1: "a",
		...
		if !reflect.DeepEqual(s, expected) {
			t.Errorf("unexpected result. expected: %v, but got: %v", expected, s)
		}
	})
}
