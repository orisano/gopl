...
func TestIntSet_IntersectWith(t *testing.T) {
	ts := []struct {
		...
		{
			a: newIntSet(1, 10, 100, 1000, 10000),
			b: newIntSet(0, 5, 10, 500, 1000, 10001),
			expected: map[int]bool{
				0:     false,
				1:     false,
				5:     false,
		...
		{
			a: newIntSet(0, 5, 10),
			b: newIntSet(1, 10, 100, 1000, 10000),
			expected: map[int]bool{
				0:     false,
				1:     false,
				5:     false,
	...
	for _, tc := range ts {
		tc.a.IntersectWith(tc.b)
		for x, expected := range tc.expected {
			if got := tc.a.Has(x); got != expected {
				t.Errorf("unexpected Has(%d). expected: %v, but got: %v", x, expected, got)
			}
		}
	}
...
func TestIntSet_Elems(t *testing.T) {
	ts := []struct {
		...
		{
			intSet:   newIntSet(100, 10, 1),
			expected: []int{1, 10, 100},
		},
		{
			intSet:   newIntSet(0, 32*1000),
			expected: []int{0, 32 * 1000},
		},
	}
	...
	for _, tc := range ts {
		if got := tc.intSet.Elems(); !equals(got, tc.expected) {
			t.Errorf("unexpected elements. expected: %v, but got: %v", tc.expected, got)
		}
	}
}
